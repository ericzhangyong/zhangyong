...
- (void)getRequestCompleted:(DataCompletionBlock)respenseBlock{
    ...
    [self getWithUrl:self.url completed:^(YHBaseRespense *respense) {
...
- (HYBURLSessionTask *)getWithUrl:(NSString*)url completed:(DataCompletionBlock)successBlock
{
    
    url = _url;
    
    
    HYBURLSessionTask *task = [HYBNetworking getWithUrl:url refreshCache:NO success:^(id response) {
...
- (YHBaseRespense *)parseFailResponse:(NSDictionary *)info {
    ...
    YHBaseRespense *baseRespense = [YHBaseRespense new];
    NSInteger code = [info[@"code"] integerValue];
    NSString *errMsg = [NSString stringWithFormat:@"%@",info[@"message"]];
    ...
    //    else if (code == 510) { //510=当前用户session已经过期
    //        //退出登录
    //        [[NSNotificationCenter defaultCenter] postNotificationName:@"loginSessionExpire" object:nil];
    //    }
