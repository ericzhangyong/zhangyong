...
#ifndef FF_SWAP
#define FF_SWAP(_a_, _b_)  do { __typeof__(_a_) _tmp_ = (_a_); (_a_) = (_b_); (_b_) = _tmp_; } while (0)
#endif
...
@implementation UIImage (FFCreate)

#pragma mark- 关于创建不同Image相关类方法

+ (UIImage *)ff_imageWithColor:(UIColor *)color {
    return [self ff_imageWithColor:color size:CGSizeMake(1, 1)];
}
...
- (UIImage *)ff_imageByResizeToSize:(CGSize)size contentMode:(UIViewContentMode)contentMode {
    if (size.width <= 0 || size.height <= 0) return nil;
    UIGraphicsBeginImageContextWithOptions(size, NO, [UIScreen mainScreen].scale);
    [self ff_drawInRect:CGRectMake(0, 0, size.width, size.height) withContentMode:contentMode clipsToBounds:NO];
    UIImage *image = UIGraphicsGetImageFromCurrentImageContext();
    UIGraphicsEndImageContext();
    return image;
}

- (UIImage *)ff_imageByCropToRect:(CGRect)rect {
...
- (void)ff_drawInRect:(CGRect)rect withContentMode:(UIViewContentMode)contentMode clipsToBounds:(BOOL)clips{
