...
@implementation FFDBModel
#pragma mark - override method
+ (void)initialize
{
    if (self != [FFDBModel self]) {
        [self createTable];
    }
}
...
+ (NSDictionary *)getPropertys
{
    ...
    NSArray *theTransients = [[self class] ff_transients];
    ...
    for (i = 0; i < outCount; i++) {
        ...
        NSString *onlyKey = [self ff_onlyKey];
        if (onlyKey) {
            if ([onlyKey isEqualToString:propertyName]) {
                sqlType = [NSString stringWithFormat:@"%@ UNIQUE",sqlType];
            }
        }
        [proTypes addObject:sqlType];
    }
    free(properties);
...
{
    __block BOOL res = NO;
    FFDBHelper *ffDB = [FFDBHelper shareInstance];
    [ffDB.dbQueue inDatabase:^(FMDatabase *db) {
...
+ (BOOL)createTable
{
    ...
    FFDBHelper *ffDB = [FFDBHelper shareInstance];
    [ffDB.dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        NSString *tableName = NSStringFromClass(self.class);
...
- (BOOL)ff_saveOrUpdate
{
    ...
    if ([primaryValue intValue] <= 0) {
        NSString *onlyKey = [self.class ff_onlyKey];
        if (onlyKey) {
            id onlyValue = [self valueForKey:onlyKey];
            if(onlyValue == nil) return NO;
            [self saveOrUpdateByColumnName:onlyKey AndColumnValue:onlyValue];
        }else{
           return [self save];
        }
...
- (BOOL)saveOrUpdateByColumnName:(NSString*)columnName AndColumnValue:(NSString*)columnValue
{
    id record = [self.class ff_findFirstByCriteria:[NSString stringWithFormat:@"where %@ = '%@'",columnName,columnValue]];
    if (record) {
...
{
    //判断是否是FFBaseModel的子类
    for (FFDBModel *model in array) {
        if (![model isKindOfClass:[FFDBModel class]]) {
            return NO;
        }
    }
...
+ (BOOL)deleteObjectsWithFormat:(NSString *)format, ...
{
    ...
    return [self ff_deleteObjectsByCriteria:criteria];
}
...
+ (NSArray *)ff_findAll
{
    ...
    [ffDB.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        while ([resultSet next]) {
            FFDBModel *model = [[self.class alloc] init];
            for (int i=0; i< model.columeNames.count; i++) {
...
+ (instancetype)findFirstWithFormat:(NSString *)format, ...
{
    ...
    return [self ff_findFirstByCriteria:criteria];
}

/** 查找某条数据 */
+ (instancetype)ff_findFirstByCriteria:(NSString *)criteria
{
    NSArray *results = [self.class ff_findByCriteria:criteria];
    if (results.count < 1) {
        return nil;
    }
...
+ (instancetype)findByPK:(int)inPk
{
    ...
    return [self ff_findFirstByCriteria:condition];
}
...
+ (NSArray *)findWithFormat:(NSString *)format, ...
{
    ...
    return [self ff_findByCriteria:criteria];
}
    ...
    [ffDB.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        while ([resultSet next]) {
            ...
            for (int i=0; i< model.columeNames.count; i++) {
                NSString *columeName = [model.columeNames objectAtIndex:i];
                NSString *columeType = [model.columeTypes objectAtIndex:i];
                if ([columeType containsString:SQLTEXT]) {
                    [model setValue:[resultSet stringForColumn:columeName] forKey:columeName];
                } else {
                    [model setValue:[NSNumber numberWithLongLong:[resultSet longLongIntForColumn:columeName]] forKey:columeName];
                }
